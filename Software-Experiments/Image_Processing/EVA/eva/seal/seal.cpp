...
//	  plain.data()[0] = 160;
//	  plain.data()[0] = plain.data()[0]+100000;
//	  plain.data()[2] = plain.data()[2]+100000;
//	  plain.data()[(plain.dyn_array().size()/2)] = plain.data()[(plain.dyn_array().size()/2)]+100000;
//	  plain.data()[(plain.dyn_array().size()/2)+2] = plain.data()[(plain.dyn_array().size()/2)+2]+100000;
//	  plain.data()[1] = 91;
//	  plain.data()[2] = 160;
//	  plain.data()[3] = 45;
//            for (std::size_t i = 4; i < plain.dyn_array().size(); i++){

//		plain.data()[i]	= 0;
//	    }
                    std::cout << "plain" << std::endl;
            for (std::size_t i = 0; i < plain.dyn_array().size(); i++){

                    std::cout << plain.data()[i] << " ";
	    }
          if (info.inputType == Type::Cipher) {
            seal::Ciphertext cipher;
            encryptor.encrypt(plain, cipher);
                    std::cout << "cipher" << std::endl;
            for (std::size_t i = 0; i < cipher.dyn_array().size(); i++){

//		cipher.data()[i]	= cipher.data()[i]+10000;
                    std::cout << cipher.data()[i] << " ";
            }		    
//	  cipher.data()[0] = cipher.data()[0]+1;
//	  cipher.data()[cipher.dyn_array().size()/2] = cipher.data()[cipher.dyn_array().size()/2]+1;
...
Valuation SEALSecret::decrypt(const SEALValuation &encOutputs,
                              const CKKSSignature &signature, double err_bit,double err_ind) {
  Valuation outputs;
  ...
  for (auto &out : encOutputs) {
    ...
    visit(Overloaded{[&](const seal::Ciphertext &cipher) {
                       ...
                       encoder.decode(plain, outputs[name],err_bit,err_ind);
                     },
