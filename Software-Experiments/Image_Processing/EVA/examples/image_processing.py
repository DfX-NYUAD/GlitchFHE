...
mult = EvaProgram('mult', vec_size=h*w)
with mult:
    image = Input('image')
    image = image*1
    Output('image', image)
...
mult.set_input_scales(25)
sobel.set_output_ranges(10)
mult.set_output_ranges(10)
...
def read_input_image():
    image = Image.open('lotus.png').convert('L')
...
def write_output_image(outputs, tag):
    ...
    enc_result_image.save(f'lotus_{tag}.png', "PNG")
...
if __name__ == "__main__":
    ...
    for prog in [sobel, harris]:
        ...
        for k in range(1):
            ...
            outputs = secret_ctx.decrypt(enc_outputs, signature,0,0)
            ...
            with open ("mse_log.txt","a+") as f:
                f.write('MSE '+str(valuation_mse(outputs, reference))+"\n")
