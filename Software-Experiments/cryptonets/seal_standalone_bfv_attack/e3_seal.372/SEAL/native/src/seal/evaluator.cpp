...
namespace seal
{
    ...
    void Evaluator::relinearize_internal(
        Ciphertext &encrypted, const RelinKeys &relin_keys, size_t destination_size, double err_ind, int flag, MemoryPoolHandle pool) const
    {
        // Verify parameters.
        ...
        SEAL_ITERATE(iter(size_t(0)), relins_needed, [&](auto I) {
            this->switch_key_inplace(
                ...
                RelinKeys::get_index(encrypted_size - 1 - I),  err_ind, flag, pool);
        });
    ...
    void Evaluator::mod_switch_scale_to_next(
        const Ciphertext &encrypted, Ciphertext &destination, double err_bit, double err_ind, int flag,MemoryPoolHandle pool) const
    {
        ...
        const Ciphertext &encrypted, Ciphertext &destination, MemoryPoolHandle pool) const
    {
        ...
        int err_ind, err_bit, flag;
        switch (context_.first_context_data()->parms().scheme())
        {
        case scheme_type::bfv:
            // Modulus switching with scaling
            mod_switch_scale_to_next(encrypted, destination,err_ind,err_bit,0, move(pool));
            break;
    ...
    void Evaluator::rescale_to_next(const Ciphertext &encrypted, Ciphertext &destination, double err_bit, double err_ind, int flag, MemoryPoolHandle pool) const
    {
        // Verify parameters.
        if (!is_metadata_valid_for(encrypted, context_) || !is_buffer_valid(encrypted))
        {
        ...
        MemoryPoolHandle pool) const
    {
        ...
        for (size_t i = 0; i < encrypteds.size() - 1; i += 2)
        {
            ...
            relinearize_inplace(temp, relin_keys, 0,0,pool);
	...
	//cout << log2(encrypted.scale()) << endl; 
	//cout << log2(plain.scale()) << endl; 
...
//	std::cout << "in multiply ntt" <<std::endl;
        // Extract encryption parameters.
        ...
        if (!is_scale_within_bounds(new_scale, context_data))
        {
	   // std::cout << "here before scale out of bounds" << std::endl;
            throw invalid_argument("scale out of bounds");
        }
        ...
        Ciphertext &encrypted, uint32_t galois_elt, const GaloisKeys &galois_keys, MemoryPoolHandle pool) const
    {
        ...
        switch_key_inplace(
            encrypted, temp, static_cast<const KSwitchKeys &>(galois_keys), GaloisKeys::get_index(galois_elt), 0, 0, pool);
#ifdef SEAL_THROW_ON_TRANSPARENT_CIPHERTEXT
        // Transparent ciphertext output is not allowed.
        if (encrypted.is_transparent())
        {
    ...
    void Evaluator::switch_key_inplace(
        Ciphertext &encrypted, ConstRNSIter target_iter, const KSwitchKeys &kswitch_keys, size_t kswitch_keys_index,double err_ind, int flag,
        MemoryPoolHandle pool) const
    {
                     //std::cout << "in switch key corruption"<< std::endl;
