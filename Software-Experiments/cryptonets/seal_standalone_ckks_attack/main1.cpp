...
try {
    ...
    size_t interval_begin = 0, interval_end = 163844;
    cout << "slots: " << n << '\n';
   ...
   // auto b8s = scale(b8d, scaler_b8);
    ...
    //xp = reshapeOrder( xp, vector<size_t>{1,2,3,0} );
    //auto xc = encryptPack<Secure,DataType>(xp);
    //auto xc = (xp);
    //xc = reshapeOrder( xc, vector<size_t>{3,0,1,2} );
   ...
   // parms.set_coeff_modulus(CoeffModulus::Create(n, { 60, 40, 40, 60 }));
    parms.set_coeff_modulus(CoeffModulus::Create(n, {60,50,40,50,40,60 }));
    ...
    //xp = reshapeOrder( xp, vector<size_t>{1,2,3,0} );
   // auto xc = ckksEncryptPack(xp,encoder, scale,encryptor);
    //Plaintext x_plain;
    //encoder.encode(xps_reshaped, scale, x_plain);
    //Ciphertext x1_encrypted;
    //encryptor.encrypt(x_plain, x1_encrypted);
    //auto xc = (xp);
    //xc = reshapeOrder( x1_encrypted[0], vector<size_t>{3,0,1,2} );
 ...
 /*   encryptors.push_back( encryptor );
    ...
    CRT<Number>::setParams(relinkeys, encoders ,decryptors, encryptors, evaluators);//,contexts);//,scale1);
/*
    EncryptionParameters p(scheme_type::ckks);
    p.set_poly_modulus_degree(n);
    p.set_coeff_modulus(CoeffModulus::Create(n,{65537, 114689, 147457, 163841, 557057}));
    cout << "here3";
    cout << "here";
...
*/
     auto b1 = crtEncode(b1d);
     auto b4 = crtEncode(b4d);
     auto b8 = crtEncode(b8d);
