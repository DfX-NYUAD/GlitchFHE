...
//    cout << "c "; io::print(shape(c));
//    cout << "b "; io::print(shape(b));
//    cout << "a "; io::print(shape(a));
//    cout << nItems << std::endl;
//    cout << nRows << std::endl;
//    cout << nCols << std::endl;
//    cout << nChannels << std::endl;
...
{
//    std::cout	<< "in add_vector" << std::endl;
    auto size = v.size();
...
template <class T, class U> vector<vector<T>>
dot1(const vector<vector<T>> & a, const vector<vector<U>> & b)
{
    auto n = a.size();
    auto m = a[0].size();
    auto p = b[0].size();
   // double error = 36595379591200000;
    if ( m != b.size() ) throw "Matrices have incompatible dimensions";
    vector<vector<T>> c( n, vector<T>(p) );
    for ( size_t i=0; i<n; i++ )
    {
        for ( size_t j=0; j<p; j++ )
        {
            ...
            for ( size_t k=0; k<m; k++ ){
                if ((i == 0)&&(j==0)&&(k==1) )  {
                std::cout << "printing here now" << std::endl; 
                //std::cout << a[i][k] << std::endl; 
                partial_res.push_back( a[i][k] ^ b[k][j] );}
                //partial_res.push_back( std::make_pair(a[i][k],error) ^ b[k][j]);}
                //partial_res.push_back(a[i][j] ^ std::make_pair(<T>(b[i][j]), error));}
                else{
                partial_res.push_back( a[i][k] * b[k][j] );}
}
            c[i][j] = add_vector(partial_res);
...
template <class T> vector<vector<vector<vector<T>>>>
reshapeOrder(const vector<vector<vector<vector<T>>>> & a, const vector<int> order)
{
    auto & o = order;
...
template <class T> vector<size_t>
shape(const vector<T> & v)
{
    vector<size_t> sizes = { v.size() };
    auto tmp = shape(v[0]);
    for (auto s : tmp) sizes.push_back(s);
    return sizes;
}
...
square1(const T & a)
{
    std::cout << "before flag check" << std::endl;
    return a ^ a;
}
...
template <class T> vector<T>
square1(const vector<T> & a)
{
    vector<T> r;
    bool isFirst = true;

    for ( const auto & e : a ) {if (isFirst) {
           r.push_back( square1(e) );
           isFirst = false;
         }
      else{
           r.push_back( square(e) );
      }
}
    return r;
}
