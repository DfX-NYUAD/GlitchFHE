...
#else
            if (flag == 1){
            std::cout << "in inverse ntt negacyclic harvey (flag = 1)" << std::endl;
            }
            else{
            std::cout << "in inverse ntt negacyclic harvey (flag = 0)" << std::endl;
            }
            MultiplyUIntModOperand inv_degree_modulo = tables.inv_degree_modulo();
            tables.ntt_handler().transform_from_rev(
                operand.ptr(), tables.coeff_count_power(), tables.get_from_inv_root_powers(), &inv_degree_modulo, flag);
#endif
        }
    } // namespace util
} // namespace seal
