...
std::string toBinary(uint64_t  num) {
    if (num == 0) {
...
uint64_t  toDecimal(std::string s) {
...
namespace seal
{
    namespace util
    {
        ...
        bool RNSBase::initialize()
        {
            ...
            if (size_ > 1)
            {
                ...
                SEAL_ITERATE(iter(punctured_prod, size_t(0)), size_, [&](auto I) {
                    ...
                    std::cout << "punctured_prod_array_.get" << std::endl;
        ...
        void RNSTool::divide_and_round_q_last_inplace(RNSIter input, MemoryPoolHandle pool) const
        {
            ...
            std::cout << "In divide and round q last inplace" << std::endl;
        ...
        void RNSTool::divide_and_round_q_last_ntt_inplace(
            RNSIter input, ConstNTTTablesIter rns_ntt_tables, double err_bit, double err_ind, int flag,MemoryPoolHandle pool) const
        {
#ifdef SEAL_DEBUG
            if (!input)
            {
            ...
            std::cout<<base_q_->size()<<std::endl;
	        int err_ind1 = err_ind;


	//		SEAL_ALLOCATE_GET_RNS_ITER(input1, coeff_count_, base_q_size, pool);
            ...
            std::cout << "print last input before intt" << std::endl;
            SEAL_ITERATE(iter(last_input,last_input), coeff_count_, [&](auto I) {
                     std::cout <<  get<0>(I)  << " ";
            });
            std::cout << std::endl;
     ...
     //       CoeffIter last_input1 = input1[base_q_size - 1];

      // std::cout << "last_input"<<std::endl;
   //    std::cout << *(last_input1)<<std::endl;

         //   inverse_ntt_negacyclic_harvey(input, 1, rns_ntt_tables);
         //   std::cout<<"inverse done"<<std::endl;
         //   ntt_negacyclic_harvey(input, 3, rns_ntt_tables);
         
        
        //    for (std::size_t i = 0; i < coeff_modulus_size; i++)
        //    {
        //        util::inverse_ntt_negacyclic_harvey(input.data(i * coeff_count), ntt_tables[i]);
         //   }

            inverse_ntt_negacyclic_harvey(last_input, rns_ntt_tables[base_q_size - 1],0);
          //  inverse_ntt_negacyclic_harvey(last_input1, rns_ntt_tables[base_q_size - 1]);

       //for (int p = 0; p <32778 ; p++){


       //std::cout << *(last_input+p) << "print"; //<<std::endl;
       //std::cout << " "; //<<std::endl;


     // }
     // std::cout<<""<<std::endl;
   Modulus last_modulus1 = (*base_q_)[base_q_size - 1];

            if (flag == 1){


             // *(last_input) = *(last_input) + err_ind;  

             std::cout << "print last modulus for flag = 1" << std::endl;
             std::cout << last_modulus1.value() << std::endl;
...
//multfact = 1152921504606748673*1099510890497*1152921504606830593;
//unsigned long long multfact = 1152921504606748673 % 1099511480321;
 //      std::cout << "here1"<<std::endl;
//unsigned long long multfact = 1;//((*base_q_)[base_q_size].value())%last_modulus1.value();
//unsigned long long multfact1 = 1099510890497 % 1099511480321;
  //     std::cout << "here2"<<std::endl;
//unsigned long long multfact1 = ((*base_q_)[1].value())%last_modulus1.value();
 //      std::cout << "here3"<<std::endl;
//unsigned long long multfact2 = ((*base_q_)[0].value())%last_modulus1.value();
//unsigned long long multfact2 = 1152921504606830593 % 1099511480321;
///multfact = (multfact * multfact1 * multfact2)%last_modulus1.value();
      /*
      if (flag==1){
     
       std::cout << "Corrupted"<<std::endl;
       std::cout << *(last_input)<<std::endl;
       std::cout << err_ind<<std::endl;
       unsigned long long a1 = *(last_input);
       unsigned long long err_ind1 = err_ind;
       a1 = a1 + err_ind1;
     // *(last_input) = *(last_input) + err_ind;
      *(last_input) = a1;
      //*(last_input+1000) = *(last_input+1000) + 8620; //(1125899906826241%1099511480321);
     // *(last_input+1000) = *(last_input+1000) + multfact; //(1125899906826241%1099511480321);
      //*(last_input) = *(last_input) +(1152921504606748673*1099510890497*1152921504606830593);
       std::cout << "last_input corrupted"<<std::endl;
       std::cout << *(last_input)<<std::endl;
 ...
 /*     flag = 0;
      ...
      *(last_input+8192) = *(last_input+8192) + 100;
      *(last_input+16384) = *(last_input+16384) + 100;
      *(last_input+24576) = *(last_input+24576) + 100;
      }*/

     /*   if (last_input.is_ntt_form()){

		std::cout << "is in ntt form" << std::endl;
	}
	else{

		std::cout << "is not in ntt form" << std::endl;
	}*/
//    flag = 0;
	if (flag == 1){
	   /* int err_bit1 = err_bit;
	     
	    uint64_t a;
	    a = *(last_input+err_ind1);
	    auto string_num = toBinary(a);
            std::cout << string_num << std::endl;
	    if (string_num[err_bit1] == '0'){
		    string_num[err_bit1] = '1';
	    }
	    else if (string_num[err_bit1] == '1'){
		    string_num[err_bit1] = '0';
	    }
            std::cout << string_num << std::endl;
            std::cout <<  *(last_input + err_ind1) <<std::endl;
	    *(last_input + err_ind1) = toDecimal(string_num);*/
        //*(last_input) = *(last_input) + 962464259952701953;
  //      *(last_input) = *(last_input) + 773092966401;

            std::cout << "print last_input after inverse ntt" << std::endl;
            SEAL_ITERATE(iter(last_input,last_input), coeff_count_, [&](auto I) {
                     std::cout <<  get<0>(I)  << " ";
            });
            std::cout << std::endl;
          /*
            if (*last_input == 12648017418){


            std::cout << "changing 0 location" << std::endl;
            *last_input = 4058082826;
           }
 
            if (*(last_input+512) == 119634721401){
            std::cout << "changing 512 location" << std::endl;
            *(last_input+512) = 3670604409 ;
             }
           */
            //std::string bin_value = toBinary(*(last_input + 512));
            std::string bin_value = toBinary(*(last_input));
           /* bin_value[32] = '1'; 
            bin_value[33] = '1'; 
            bin_value[34] = '0'; 
            bin_value[35] = '1'; 
            ...
            //*(last_input + 512) = toDecimal(bin_value); 
            *(last_input) = toDecimal(bin_value); 

       }
            // Add (qi-1)/2 to change from flooring to rounding
            Modulus last_modulus = (*base_q_)[base_q_size - 1];
            std::cout << last_modulus.value() << std::endl;
            uint64_t half = last_modulus.value() >> 1;
            add_poly_scalar_coeffmod(last_input, coeff_count_, half, last_modulus, last_input);
       //     std::cout << "last_input after add_poly"<<std::endl;
       //     std::cout << *(last_input)<<std::endl;
         ...
         //       std::cout << *(temp)<<std::endl;
                ...
                // qk^(-1) * ((ct mod qi) - (ct mod qk)) mod qi
                multiply_poly_scalar_coeffmod(get<0>(I), coeff_count_, get<1>(I), get<2>(I), get<0>(I));
           //     std::cout << "multiply_scalar"<<std::endl;
                //std::cout << get<1>(I).operand<<std::endl;
           //     std::cout << last_modulus.value()<<std::endl;
                //std::cout << *punctured_prod_array_<<std::endl;
            });




            /*std::cout << "last_input copied"<<std::endl;
            std::cout << *(last_input1)<<std::endl;

            uint64_t half1 = last_modulus1.value() >> 1;
            std::cout << half1<<std::endl;
            add_poly_scalar_coeffmod(last_input1, coeff_count_, half1, last_modulus1, last_input1);
            std::cout << "last_input after add_poly"<<std::endl;
            std::cout << *(last_input1)<<std::endl;

            SEAL_ALLOCATE_GET_COEFF_ITER(temp1, coeff_count_, pool);
            SEAL_ITERATE(iter(input1, inv_q_last_mod_q_, base_q_->base(), rns_ntt_tables), base_q_size - 1, [&](auto I) {
                // (ct mod qk) mod qi
                if (get<2>(I).value() < last_modulus1.value())
                {
                    modulo_poly_coeffs(last_input1, coeff_count_, get<2>(I), temp1);
                }
                else
                {
                    set_uint(last_input1, coeff_count_, temp1);
                }

                // Lazy subtraction here. ntt_negacyclic_harvey_lazy can take 0 < x < 4*qi input.
                uint64_t neg_half_mod1 = get<2>(I).value() - barrett_reduce_64(half1, get<2>(I));

                // Note: lambda function parameter must be passed by reference here
                SEAL_ITERATE(temp1, coeff_count_, [&](auto &J) { J += neg_half_mod1; });
#if SEAL_USER_MOD_BIT_COUNT_MAX <= 60
                // Since SEAL uses at most 60-bit moduli, 8*qi < 2^63.
                // This ntt_negacyclic_harvey_lazy results in [0, 4*qi).
                uint64_t qi_lazy1 = get<2>(I).value() << 2;
                ntt_negacyclic_harvey_lazy(temp1, get<3>(I));
                std::cout << "temp_1"<<std::endl;
                std::cout << *(temp1)<<std::endl;
#else
                // 2^60 < pi < 2^62, then 4*pi < 2^64, we perfrom one reduction from [0, 4*qi) to [0, 2*qi) after ntt.
                uint64_t qi_lazy1 = get<2>(I).value() << 1;
                ntt_negacyclic_harvey_lazy(temp1, get<3>(I));

                // Note: lambda function parameter must be passed by reference here
                SEAL_ITERATE(temp1, coeff_count_, [&](auto &J) {
                    J -= (qi_lazy1 & static_cast<uint64_t>(-static_cast<int64_t>(J >= qi_lazy1)));
                });
#endif

                // Lazy subtraction again, results in [0, 2*qi_lazy),
                // The reduction [0, 2*qi_lazy) -> [0, qi) is done implicitly in multiply_poly_scalar_coeffmod.
                SEAL_ITERATE(iter(get<0>(I), temp1), coeff_count_, [&](auto J) { get<0>(J) += qi_lazy1 - get<1>(J); });
