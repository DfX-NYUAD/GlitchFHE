...
namespace seal
{
    ...
    namespace util
    {
        ...
        public:
            ...
            inline std::complex<double> mul_root_scalar(const std::complex<double> &r, const double &s, int flag ) const
            {
                  if (flag == 0){

                  std::cout << "corrupting intermediate (flag = 0)" << std::endl;
                  return r * s;
                  }
                  else{
                  std::cout << "corrupting intermediate (flag = 1)" << std::endl;
                  std::complex<double> intermediate = r*s;
                  const double l = 2;
    ...
    */
    ...
    private:
            ...
            MemoryPoolHandle pool)
        {
            ...
            fft_handler_.transform_from_rev(conj_values.get(), util::get_power_of_two(n), inv_root_powers_.get(), &fix,0);
	    ...
	    std::cout<<"ntt" << std::endl;
            for (std::size_t i = 0; i < coeff_modulus_size; i++)
            {
                util::inverse_ntt_negacyclic_harvey(plain_copy.get() + (i * coeff_count), ntt_tables[i],0);
	        std::cout << ntt_tables[i].get_root() << std::endl;
            }
            std::cout << " here1 in decode" << std::endl;
	        //  plain_copy.get()[0] = plain_copy.get()[0]+1;
