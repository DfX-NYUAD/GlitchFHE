...
namespace seal
{
	...
	cout << "operand check in add_in_place" << endl;
        // Add ciphertexts
    ...
    void Evaluator::bfv_multiply(Ciphertext &encrypted1, const Ciphertext &encrypted2, MemoryPoolHandle pool)
    {
        ...
        inverse_ntt_negacyclic_harvey_lazy(temp_dest_q, dest_size, base_q_ntt_tables,0);
        inverse_ntt_negacyclic_harvey_lazy(temp_dest_Bsk, dest_size, base_Bsk_ntt_tables,0);
    ...
    void Evaluator::bfv_square(Ciphertext &encrypted, MemoryPoolHandle pool)
    {
        ...
        // Perform BEHZ step (5): transform data from NTT form
        inverse_ntt_negacyclic_harvey(temp_dest_q, dest_size, base_q_ntt_tables,0);
        inverse_ntt_negacyclic_harvey(temp_dest_Bsk, dest_size, base_Bsk_ntt_tables,0);
    ...
    void Evaluator::relinearize_internal(
        Ciphertext &encrypted, const RelinKeys &relin_keys, size_t destination_size, double err_bit, double err_ind, int flag, MemoryPoolHandle pool)
    {
        // Verify parameters.
	//
	    //std::cout << "here in relinearize" << std::endl;
        ...
        SEAL_ITERATE(iter(size_t(0)), relins_needed, [&](auto I) {
            this->switch_key_inplace(
                ...
                RelinKeys::get_index(encrypted_size - 1 - I), err_bit, err_ind, flag, pool);
        });
    ...
    void Evaluator::mod_switch_scale_to_next(
        const Ciphertext &encrypted, Ciphertext &destination,double err_bit, double err_ind, int flag, MemoryPoolHandle pool)
    {
...
std::cout << "encrypted_size" << std::endl;
	std::cout << encrypted_size << std::endl;
	std::cout << encrypted.dyn_array().size() << std::endl;
    std::cout << parms.poly_modulus_degree() << std::endl;
    std::cout << parms.coeff_modulus().size() << std::endl;
        Ciphertext encrypted_copy(pool);
        encrypted_copy = encrypted;
        std::cout << "check coeff modulus" << std::endl;
        ...
        case scheme_type::ckks:
            SEAL_ITERATE(iter(encrypted_copy), encrypted_size, [&](auto I) {
               ...
               std::cout << "here higher i" <<std::endl;
               std::cout << i << std::endl; 
               flag = 0;
               }
               rns_tool->divide_and_round_q_last_ntt_inplace(I, context_data.small_ntt_tables(), err_bit,err_ind,flag,pool);
               
               i = i + 1;
             });
	    std::cout << "ckks here" <<std::endl;
            break;
	...
	int flag = 0;
        ...
        case scheme_type::bfv:
            ...
            mod_switch_scale_to_next(encrypted, destination, err_bit,err_ind,flag,move(pool));
            break;
    ...
    void Evaluator::rescale_to_next(const Ciphertext &encrypted, Ciphertext &destination, double err_bit, double err_ind, int flag,MemoryPoolHandle pool)
    {
        // Verify parameters.
        if (!is_metadata_valid_for(encrypted, context_) || !is_buffer_valid(encrypted))
        {
    ...
    void Evaluator::rescale_to_inplace(Ciphertext &encrypted, parms_id_type parms_id, MemoryPoolHandle pool)
    {
	 ...
	 int flag = 0;
        ...
        case scheme_type::ckks:
            ...
            {
                // Modulus switching with scaling
                mod_switch_scale_to_next(encrypted, encrypted,err_bit,err_ind,flag, pool);
            }
            break;
        ...
        MemoryPoolHandle pool)
    {
	    ...
	    double err_ind, err_bit;
	    int flag = 0;
            relinearize_inplace(temp, relin_keys,err_bit, err_ind, flag, pool);
        ...
        */
        ...
        SEAL_ITERATE(iter(encrypted), encrypted_size, [&](auto I) {
            SEAL_ITERATE(iter(I, temp_iter, coeff_modulus, ntt_tables), coeff_modulus_size, [&](auto J) {
                ...
                inverse_ntt_negacyclic_harvey(get<0>(J), get<3>(J),0);
            });
        });
    ...
    void Evaluator::transform_from_ntt_inplace(Ciphertext &encrypted_ntt)
    {
        ...
        inverse_ntt_negacyclic_harvey(encrypted_ntt, encrypted_ntt_size, ntt_tables,0);
	...
	int flag = 0;
        ...
        switch_key_inplace(
            encrypted, temp, static_cast<const KSwitchKeys &>(galois_keys), GaloisKeys::get_index(galois_elt), err_bit, err_ind, flag, pool);
#ifdef SEAL_THROW_ON_TRANSPARENT_CIPHERTEXT
        // Transparent ciphertext output is not allowed.
        if (encrypted.is_transparent())
        {
    ...
    void Evaluator::switch_key_inplace(
        Ciphertext &encrypted, ConstRNSIter target_iter, const KSwitchKeys &kswitch_keys, size_t kswitch_keys_index, double err_bit, double err_ind, int flag,
        MemoryPoolHandle pool)
    {
        ...
        {
            inverse_ntt_negacyclic_harvey(t_target, decomp_modulus_size, key_ntt_tables,0);
        }
        std::cout << "decomp_modulus_size" << std::endl;
        std::cout << decomp_modulus_size << std::endl;
        // Temporary result
        ...
        SEAL_ITERATE(iter(encrypted, t_poly_prod_iter), key_component_count, [&](auto I) {
            ...
            inverse_ntt_negacyclic_harvey_lazy(t_last, key_ntt_tables[key_modulus_size - 1],0);


            //*t_last = *t_last  + 104555417601;
            ...
            SEAL_ITERATE(iter(I, key_modulus, key_ntt_tables, modswitch_factors), decomp_modulus_size, [&](auto J) {
                ...
                {
                    inverse_ntt_negacyclic_harvey_lazy(get<0, 1>(J), get<2>(J),0);
                   
                     if (flag == 1){
                     *get<0, 1>(J) = *get<0, 1>(J) + 104555417601*err_ind;
                     }
                     //*get<0, 1>(J) = *get<0, 1>(J) + 62008418304;
                }
