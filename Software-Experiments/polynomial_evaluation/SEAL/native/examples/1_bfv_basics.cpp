...
double calculateMSE(const std::string& str1, const std::string& str2) {
    ...
    // Calculate squared differences for overlapping characters
    for (size_t i = 0; i < std::min(len1, len2); ++i) {
        double diff = static_cast<double>(str1[i]) - static_cast<double>(str2[i]);
        mse += diff * diff;
    }

    // Treat remaining characters in the longer string as having an implicit match with 0
    if (len1 > len2) {
        for (size_t i = len2; i < len1; ++i) {
            mse += static_cast<double>(str1[i]) * static_cast<double>(str1[i]);
        }
    } else if (len2 > len1) {
        for (size_t i = len1; i < len2; ++i) {
            mse += static_cast<double>(str2[i]) * static_cast<double>(str2[i]);
        }
    }

    mse /= maxLen; // Normalize by the length of the longer string
    return mse;
}
...
{
    print_example_banner("Example: Attacking BFV");

    int flag = 1;  //Flag is 0 when fault-injection is not enabled, else 1
    std::string filename = "output_bfv_attack.txt";
    ...
    */
    ...
    int err_mult;
    std::string orig1, orig2, orig3, orig4, orig5, orig6;
   
    //err_mult is the multiplication factor to be multiplied with error to be induced. This value can be varied as requirement.
 
    for (err_mult = 0; err_mult <100000; err_mult=err_mult+100){
    ...
    */
    print_line(__LINE__);
    Ciphertext x_encrypted,x_encrypted1;
    ...
    */
    Plaintext x_decrypted,x_decrypted1;
    cout << "    + decryption of x_encrypted: ";
    decryptor.decrypt(x_encrypted, x_decrypted);
    cout << "0x" << x_decrypted.to_string() << " ...... Correct." << endl;
    if (err_mult == 0){
       orig1 =  x_decrypted.to_string();
    }
    ...
    */
    ...
    //Adding Fault Here, err_mult indicates the multiplier value for the induced error
    // If the last argument of the relinearize_inplace is 1, it indicates insertion of fault, else no fault injection 

    evaluator.relinearize_inplace(x_squared, relin_keys,err_bit,err_mult,1);
    ...
    if (err_mult == 0){
       orig2 =  decrypted_result.to_string();
    }
    else{


    if (orig2 != decrypted_result.to_string()){

    outputFile << "Not-Equal" << " ";    
    ...
    outputFile << decryptor.invariant_noise_budget(x_sq_plus_one) << "bits ";    
    ...
    // If the last argument of the relinearize_inplace is 1, it indicates insertion of fault, else no fault injection 
    //Here, we are not inserting fault, to enable fault injection at this stage, give an input of 1

    evaluator.relinearize_inplace(x_plus_one_sq, relin_keys,err_bit,err_mult,0);
    cout << "    + noise budget in x_plus_one_sq: " << decryptor.invariant_noise_budget(x_plus_one_sq) << " bits"
         << endl;
    cout << "    + decryption of x_plus_one_sq: ";
    decryptor.decrypt(x_plus_one_sq, decrypted_result);
    ...
    if (err_mult == 0){
       orig3 =  decrypted_result.to_string();
    }
    else{


    if (orig3 != decrypted_result.to_string()){
    ...
    outputFile << decryptor.invariant_noise_budget(x_plus_one_sq) << "bits ";    
    ...
    evaluator.relinearize_inplace(encrypted_result, relin_keys,err_bit,err_mult,0);
    ...
    */
    ...
    if (err_mult == 0){
       orig4 =  decrypted_result.to_string();
    }


    else{
    mse = calculateMSE(orig4, decrypted_result.to_string());
    outputFile << mse << " ";    
        std::cout << "Mean Square Error: " << mse << std::endl;
        outputFile  << mse << " ";
    if (orig4 != decrypted_result.to_string()){
    ...
    outputFile << decryptor.invariant_noise_budget(encrypted_result) << "bits " << endl;    
